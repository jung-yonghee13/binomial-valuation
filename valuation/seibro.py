...
def fetch_treasury_curve(
    ...
    timeout: float = 10,
    retry_timeout: float = 4,
    total_deadline: float = 20,
) -> dict:
    ...
    소급 매달림 방지: 첫 조회는 timeout(기본 10s), 이후 소급 재조회는 짧은
    retry_timeout(기본 4s)으로 잡고, 전체 소급에 total_deadline(기본 20s)의
    총 예산을 둔다. 느린 응답 + 국고채권 행 부재가 겹쳐도 최악 8×10=80s가 아니라
    약 20s 안에 실패해 상위(resolve_risk_free)가 스냅샷 폴백으로 넘어가게 한다.
    (정상 동작·휴일 소급 로직 자체는 그대로 유지)
    """
    ...
    for back in range(max_lookback_days + 1):
        ...
        step_timeout = timeout if back == 0 else retry_timeout
        # 남은 총 예산과 각 회 timeout 중 작은 값으로 제한 (총 매달림 상한 보장)
        remaining = deadline - time.monotonic()
        if back > 0 and remaining <= 0:
            break
        row = _parse_treasury_row(
            _fetch_xml(used.strftime("%Y%m%d"), timeout=max(1.0, min(step_timeout, remaining)))
        )
        if row:
            break
    if not row:
        raise RuntimeError(
            ...
            "찾지 못했습니다(또는 소급 시간 예산 초과). 날짜를 확인하거나 수동 곡선 JSON을 사용하세요."
        )

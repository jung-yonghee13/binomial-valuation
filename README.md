...
콜옵션 등 옵션성 증권의 공정가치 평가를 **데이터 수집부터 PDF 평가보고서까지 자동화**하는 프로젝트입니다.

계약조건과 피어그룹만 입력하면 — 피어 주가와 국고채 수익률을 자동 수집해 변동성·무위험이자율을 산출하고, CRR 이항모형으로 평가한 뒤, 몬테카를로로 교차검증하고, 평가보고서(PDF)를 생성합니다.
...
> **배포**: Streamlit Community Cloud에 GitHub 저장소를 연결하면 push할 때마다 자동 재배포됩니다.
> 리눅스 배포 환경 대응(PDF 변환용 chromium, 한글 폰트)은 `packages.txt`에 포함되어 있습니다.

전환사채(CB), 상환전환우선주(RCPS) 등 옵션이 내재된 증권의 평가는 실무에서 주로 엑셀로 수행되지만, 트리 스텝이 촘촘해지거나 전환가액 조정(리픽싱)·조기상환권 같은 조건이 붙으면 관리가 어렵고 느려집니다. 이 프로젝트는 평가 로직을 파이썬으로 구현하여 **빠르고, 검증 가능하고, 재현 가능한 평가 파이프라인**을 만드는 것을 목표로 합니다.
...
각 모듈의 산출물이 다음 모듈로 무손실 전달되는 것을 자동 테스트(pytest 58개)와 독립 재계산 진단으로 보증합니다. 같은 입력이면 언제나 같은 결과가 나옵니다(난수 시드 고정 포함).
...
`streamlit run app.py` 실행 후, 좌측에 입력하고 **가치평가 실행**을 누르면 우측 패널에 결과가 표시됩니다.

| 사용자가 입력 | 자동으로 처리 |
|---------------|---------------|
| 계약조건 (계약명·당사자·기초자산·수량·행사가격·만기·행사방식) — 직접 입력 또는 계약정보 JSON 첨부 | 피어 주가 수집 → 변동성 산출·평균 |
| 평가기준일, 기초자산 가액(+추정 근거), 배당 여부 | Seibro 국고채 수익률 수집 → spot rate → 선도이자율 |
| **피어그룹** (기업명·종목코드) | CRR 이항모형 평가 → 몬테카를로 교차검증 → 민감도 분석 |
| 계산 설정 (할인 방식·트리 스텝·MC 경로 수) | PDF 평가보고서 생성 |

우측 미리보기: 핵심 지표(1주당 가치·총 평가액·교차검증), 보고서 실물 미리보기, 변동성·선도이자율 산출 내역, 민감도 분석, 계산 과정 JSON. 결과 JSON과 PDF 보고서를 바로 내려받을 수 있습니다.
...
- [x] PDF 평가보고서 자동 생성 (가정·산출 근거·트리·민감도 수록)
- [x] 웹 대시보드 (입력 → 실행 → 결과·보고서 미리보기 → 다운로드)
- [x] 검증 자동화 (pytest 58개 — 수렴·패리티·경계조건·교차검증·파이프라인)
- [x] 평가 워크플로 스킬 + 외부평가업무 행동 강령 문서화

**진행 예정**

- [ ] **스텝별 행사가격 스케줄**: 페이오프를 시점 의존 함수로 확장 — 전환가액 조정(리픽싱) 등 시간에 따라 변하는 계약 조건 반영
- [ ] **LSMC 확장**: Longstaff-Schwartz 회귀 기반 몬테카를로 — 미국형 조기행사 교차검증 및 경로의존 조건 평가
